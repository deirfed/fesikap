...
class CekStatusAktivitas extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->desa_id) {
            $visit = Visit::where('project_id', $project_id)
                ->where('desa_id', $this->desa_id)
                ->orderBy('date', 'DESC')
                ->first();
            $desa_visit = Desa::find($this->desa_id);
        }

        return view('livewire.cek-status-aktivitas', compact([
            'kabupaten',
            'kecamatan',
            'desa',
            'visit',
